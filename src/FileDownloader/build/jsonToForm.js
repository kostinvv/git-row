(function ($) {
    $.fn.jsonToForm = function (options) {
        ...
        function addArrayItem(arrayContainer, needInitiations, itemIndex) {
            let tId = arrayContainer.attr("data-template-id");
            let htmlTemplate = arrayTemplates[tId]["htmlTemplate"];
            let dataTemplate = JSON.parse(JSON.stringify(arrayTemplates[tId]["dataTemplate"]));
            let dataPath = arrayContainer.parents("tr:first").next().find("td:first").attr("data-path");
        
            // Преобразование dataPath в массив ключей
            let pathArray = dataPath.replace(/\['([^']+?)'\]/g, '.$1').split('.').filter(Boolean);
        
            // Проверка и инициализация пустого массива, если нужно
            if (!getValueByPath(options["value"], pathArray)) {
                setValueByPath(options["value"], pathArray, []);
            }
        
            if (itemIndex == null) {
                let arr = getValueByPath(options["value"], pathArray);
        ...
        /*function addArrayItem(arrayContainer, needInitiations, itemIndex) {
            // Получаем идентификатор шаблона и данные
            let tId = arrayContainer.attr("data-template-id");
            let htmlTemplate = arrayTemplates[tId]["htmlTemplate"];
            let dataTemplate = JSON.parse(JSON.stringify(arrayTemplates[tId]["dataTemplate"]));
            
            // Получаем путь к массиву
            let dataPath = arrayContainer.parents("tr:first").next().find("td:first").attr("data-path");
            
            // Получаем массив по пути
            let arr = getValueByPath(options["value"], dataPath);
            
            // Инициализируем массив, если он не существует
            if (!Array.isArray(arr)) {
                arr = [];
                setValueByPath(options["value"], dataPath, arr);
            }
            ...
            if (itemIndex == null) {
                ...
                arr[itemIndex] = dataTemplate;
            }
            
            // Обновляем HTML шаблон с новым индексом
            htmlTemplate = replaceAll(htmlTemplate, "$index$", itemIndex);
            arrayContainer.parents("tr:first").next().find("td:first").append(htmlTemplate);
        ...
        function setValueByPath(obj, path, value) {
            ...
            let last = parts.pop();
            let lastObj = parts.reduce((acc, part) => acc[part] = acc[part] || {}, obj);
            lastObj[last] = value;
        }*/
        ...
        /*function addArrayItemsToTheDOM() {
            ...
            arrayNodes.each(function () {
                ...
                if (dataPath === undefined) {
                    ...
                    dataPath = o.attr("data-path");
                }
        
                var arr = getValueByPath(options["value"], dataPath);
            ...
            function addArrayItemsToTheDOM() {
                ...
                arrayNodes.each(function () {
                    ...
                    try {
                        // Преобразование строки пути в массив для использования в функции getValueByPath
                        var pathArray = dataPath.replace(/\['([^']+?)'\]/g, '.$1').split('.').filter(Boolean);
                        var arr = getValueByPath(options["value"], pathArray);
                        ...
                        console.log("Ошибка доступа к данным по пути: ", dataPath, err);
                    }
